...
def amountize(io):
    ...
    '¥5361'
    """
    process_words = helpers.processing(io)
    if len(set(process_words) & set(voc.currencies.keys())):
        currency = set(process_words) & set(
            voc.currencies.keys())  # raise exception /try except
...
def abbreviationize(input_str, sep=""):
    ...
    'Baby, Baby, Baby, Baby' 
    """
    process_words = helpers.processing(input_str)
    ...
    if len(intersection):
        ...
        # lstrip() can also be used, but strip trims spaces from both sides
        output = (item*times_num).strip()
